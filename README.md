...
- **開発の背景・経緯**
  従来の人狼や、それを単に模倣したデジタルアプリでは「プレイヤーが嘘をつく」ことはあっても、「ゲームのシステム（UI）自体がプレイヤーに嘘をつく」ことは不可能であった。
...
**1. システム接続と役職編成（待機ロビー）**
<img width="1920" height="1032" alt="スクリーンショット 2026-02-22 155902" src="https://github.com/user-attachments/assets/836c3aeb-d1e7-4581-bc77-47ae5218c6ca" />

プレイヤーはモデル名を入力してシステムに接続する。ホストは参加人数に合わせて「Malware（人狼）」「Stable（村人）」「Trojan（てるてる）」などの独自役職の枚数を自在に設定し、ゲームを開始することができる。

**2. 夜の行動フェーズ（ハルシネーション）**
<img width="1920" height="1032" alt="スクリーンショット 2026-02-22 161007" src="https://github.com/user-attachments/assets/e0d410f6-fd41-4ec7-985b-de19639893d8" />

  夜フェーズでは各自に役職が割り当てられるが、毎ゲームランダムに1名のモデル（プレイヤー）が「ハルシネーション（AIの異常出力）」状態となる。エラーを起こしているモデルには真の役職とは全く異なる役職が表示され、対象をスキャン（占い）した場合も、システムからもっともらしい「嘘の結果（偽装データ）」が返却される。

**3. 昼の議論フェーズとリアルタイム同期タイマー**
<img width="1920" height="1032" alt="スクリーンショット 2026-02-22 161018" src="https://github.com/user-attachments/assets/8458f338-cf59-4f8d-b56b-485253101a38" />
...
<img width="1920" height="1032" alt="スクリーンショット 2026-02-22 161026" src="https://github.com/user-attachments/assets/7ada600e-47cf-4dc4-873f-82de8e3a6ff7" />

**5. 最終リザルト画面**
<img width="1920" height="1032" alt="スクリーンショット 2026-02-22 161044" src="https://github.com/user-attachments/assets/723bca91-7f8a-465d-a1dd-346939fe0a8a" />

  勝敗が決すると、全プレイヤーの「最終的な真の役職」と「投票先」が一覧で表示される。ここで初めて、どのモデルがハルシネーションを起こし、何の役職だと誤認（自認）していたのかが明かされる。自分の画面すら信じられない状態からの種明かしは、感想戦を最高に盛り上げる。
...
* **「嘘のデータ」を矛盾なく生成するロジック**
ハルシネーション対象者が「占い師（Scanner）」や「怪盗（Scraper）」として誤出力（偽装データ）を受け取っている場合、システムはそれに合わせて「ランダムな嘘の役職データ」を即座に生成して返す必要がある。プレイヤーに「自分がバグっている」と気づかせないよう、自然なレスポンスを組み立てる条件分岐の設計に工夫を凝らした。
...
現在はフロントエンド主体で状態管理を行っているため、熟練のユーザーによるブラウザの開発者ツールを通じたデータ傍受を防ぐため、次期アップデートにてSupabaseのRow Level Security (RLS) を導入した完全なアクセス制御や、計算ロジックのサーバーサイド移行を実装予定である。
* **UI改善**
議論時間の際、役職の名乗りや、役職の効果実行結果をログやボタンで表示させることで、快適な議論が可能になるよう、調整中である。
